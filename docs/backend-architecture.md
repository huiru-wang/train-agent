...
│       └── SKILL.md
├── static/                  # 静态资源
│   └── ppt-styles/          #   系统预设 PPT 风格 HTML 文件（14 种）
├── scripts/                 # 辅助脚本
...
| `GET` | `/api/ppt-style-preview/{path}` | 风格预览 HTML | 系统/自定义统一端点 |
...
- **PPT 风格数据库化**：系统预设风格在 DB 初始化时 seed，自定义风格通过风格提取工作流生成
...
llm = ChatOpenAI(...)  # 用于文档摘要生成和风格提取 (SUMMARIZATION_MODEL)
doc_service = DocManager(db=db, vector_store=vector_store, file_store=file_store, llm=llm)
style_extract_manager = StyleExtractManager(db=db, file_store=file_store, llm=llm)
```
...
`PromptManager` 类定义 Agent 的角色为 **企业培训专家**，核心约束包括：
- 结构化 Markdown 输出
- 基于文档事实，禁止捏造
- 引用规范（`{{ref:文档名|章节}}`）
...
此外，`PromptManager` 还提供风格提取相关的提示词构建方法：
- `build_style_description_prompt()` — 构建风格描述生成提示词
- `build_preview_html_prompt()` — 构建预览 HTML 生成提示词
...
| `TrainAgentSummarizationMiddleware` | after_agent | 长对话摘要压缩（token 超 40000 时触发） |
...
Agent 注册了 **7 个工具**，通过 `create_tools(ctx)` 统一注册：
...
- **五张表**：
  ...
  - `ppt_style(id, user_id, category, name, name_en, description, style_description, preview_path, created_at)` — PPT 风格预设与自定义
- **外键**：document/task → workspace（ON DELETE CASCADE），task.parent_task_id → task（ON DELETE SET NULL）
- **ppt_style 初始化**：DB 初始化时自动 seed 14 种系统预设风格（`user_id="system"`，`category` 为 `dark`/`light`）
...
- **产出路径**：`{workspace_id}/outputs/{task_id}/`（PPT、口播稿、风格提取产出统一按任务 ID 隔离）
...
12. **PPT 风格数据库化**：系统预设风格和用户自定义风格统一存储在 `ppt_style` 表，支持动态查询和中间件注入
