...
保存后，**必须**执行打包脚本将外部资产内联，生成完全独立的单文件：

```
terminal(command="python3 ${SKILL_DIR}/scripts/bundle.py <save_output返回的文件路径>")
```

打包脚本会自动将 `./assets/` 的 CSS/JS 引用替换为内联内容，原地覆盖文件。
...
- `references/full-decks.md` — 15 个完整 deck 模板
